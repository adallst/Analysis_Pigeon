...
%             mdl.Coefficients.Estimate(2)-mdl.Coefficients.SE(2)];
        nexttile; hold on
        use =~isnan(bData(:,6:end,bb,1,1));
        thisbdata = bData(:,6:end,bb,1,1);
%         thisRTs = repmat(RTs(4:end),numSubjects,1);
        thisDTs = repmat(DTs(6:end),numSubjects,1);
...
%         plot(find(Lp), 2.*ones(sum(Lp),1), 'mo', 'MarkerFaceColor', 'm');
        ...
        plot(DTs(6:end),lateslopes(bb,1).*DTs(6:end)+lateslopes(bb,2),'r')

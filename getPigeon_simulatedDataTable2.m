function dataTable_ = getPigeon_simulatedDataTable2(specs)
%
% Returns:
...
for bb = 1:length(blockStructs)
    ...
    for ss = 1:blockArgs(bb).numSubjects
        ...
        % Do the sim
        [choices, rts, DV, bounds, ndts,dts] = getPigeon_simulatedData2(args{:});
        ...
        dataTable_.DT(inds) = dts(1:trialCount);
...
%                 dataTable_.boundHi(inds(Lrt)) = DV(Lrt,rr-r_ndt);
                dataTable_.bound(inds(Lrt)) = mean(DV(Lrt,rr-1+[0 1]),2);
                dataTable_.boundLo(inds(Lrt)) = DV(Lrt,rr-1);
                dataTable_.boundHi(inds(Lrt)) = DV(Lrt,rr);
...
dataTable_.bound(dataTable_.bound>0.75)=0.75;
dataTable_.boundLo(dataTable_.boundLo>0.75)=0.75;
dataTable_.boundHi(dataTable_.boundHi>0.75)=0.75;
